...
async function salvar(conn, conversaId, papel, dados = {}) {
  ...
  await conn.execute(
    ...
    { c: conversaId, n, papel,
      conteudo: { val: dados.texto || '', type: tipos.DB_TYPE_CLOB },
      tj: { val: temTool ? JSON.stringify({ toolCallId: dados.toolCallId, nome: dados.nome, args: dados.args, resultado: dados.resultado }) : null, type: tipos.DB_TYPE_CLOB } });
}
