 ...
 * @param {string} [phoneNumberId] Número de origem; default = WA_PHONE_NUMBER_ID do .env.
 * @returns {Promise<object>} Resposta JSON da Meta (contém messages[].id = wamid).
 */
async function sendMessage(body, phoneNumberId) {
...
async function graphGet(pathOrUrl) {
  ...
  const res = await fetchMeta(url, { headers: { Authorization: `Bearer ${cfg.waToken}` } });
  if (!res.ok) {
    const json = await res.json().catch(() => ({}));
...
async function graphPost(pathOrUrl, body) {
  const url = pathOrUrl.startsWith('http') ? pathOrUrl : `${BASE}/${pathOrUrl}`;
  const res = await fetchMeta(url, {
    method: 'POST',
...
module.exports = { sendMessage, graphGet, graphPost, cfg, BASE };
